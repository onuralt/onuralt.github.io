    ...
    function updateRobotProgress() {
      ...
      const reveal = (value, start, span) => Math.min(Math.max((value - start) / span, 0), 1);
      ...
      robotChapters.forEach((chapter) => {
        ...
        const coverX = Math.round(progress * viewport * -0.36);
        ...
        const captionRgbY = progress < 0.34 ? Math.round((1 - ballRgb) * 36) : Math.round(-90 * Math.min(Math.max((progress - 0.34) / 0.16, 0), 1));
        const captionRgbOpacity = progress < 0.34 ? ballRgb : 1 - Math.min(Math.max((progress - 0.34) / 0.16, 0), 1);
        const captionHsvIn = Math.min(Math.max((progress - 0.34) / 0.16, 0), 1);
        const captionHsvOut = Math.min(Math.max((progress - 0.62) / 0.16, 0), 1);
        const captionHsvY = captionHsvOut > 0 ? Math.round(-90 * captionHsvOut) : Math.round((1 - captionHsvIn) * 36);
        const captionHsvOpacity = captionHsvOut > 0 ? 1 - captionHsvOut : captionHsvIn;
        const captionThresholdIn = Math.min(Math.max((progress - 0.62) / 0.16, 0), 1);
        const captionThresholdOut = Math.min(Math.max((progress - 0.8) / 0.16, 0), 1);
        const captionThresholdOpacity = captionThresholdIn * (1 - captionThresholdOut);
        const captionThresholdY = captionThresholdOut > 0 ? Math.round(-90 * captionThresholdOut) : Math.round((1 - captionThresholdIn) * 36);
        const filterStackX = Math.round(((-0.12 * (1 - ballStack)) + (0.025 * ballStack)) * viewportWidth);
        ...
        const flowSteps = [0.43, 0.5, 0.57, 0.64, 0.71, 0.78, 0.85, 0.92].map((start) => reveal(progress, start, 0.06));
        const flowLines = [0.48, 0.55, 0.62, 0.69, 0.76, 0.83, 0.9, 0.96].map((start) => reveal(progress, start, 0.05));
        ...
        chapter.style.setProperty("--caption-rgb-opacity", captionRgbOpacity.toFixed(4));
        chapter.style.setProperty("--caption-rgb-y", `${captionRgbY}px`);
        chapter.style.setProperty("--caption-hsv-opacity", captionHsvOpacity.toFixed(4));
        chapter.style.setProperty("--caption-hsv-y", `${captionHsvY}px`);
        chapter.style.setProperty("--caption-threshold-opacity", captionThresholdOpacity.toFixed(4));
        chapter.style.setProperty("--caption-threshold-y", `${captionThresholdY}px`);
        ...
        flowSteps.forEach((value, index) => chapter.style.setProperty(`--flow-${index + 1}`, value.toFixed(4)));
        flowLines.forEach((value, index) => chapter.style.setProperty(`--flow-line-${index + 1}`, value.toFixed(4)));
        ...
        chapter.style.setProperty("--threshold-x", `${Math.round(ballStack * 48)}px`);
      ...
      vlsiChapters.forEach((chapter) => {
        ...
        const travel = Math.max(rect.height - viewport, 1);
        const progress = Math.min(Math.max(-rect.top / travel, 0), 1);
        const isSequence = chapter.classList.contains("vlsi-comparison-panel")
        ...
        chapter.style.setProperty("--vlsi-rail-width", `${(progress * 100).toFixed(2)}%`);
        ...
        chapter.style.setProperty("--vlsi-media-y", `${Math.round((1 - media) * 42)}px`);
        chapter.style.setProperty("--vlsi-text-y", `${Math.round((1 - text) * 36)}px`);
